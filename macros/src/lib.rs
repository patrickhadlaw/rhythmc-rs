#![feature(custom_inner_attributes, proc_macro_span, proc_macro_diagnostic, proc_macro_def_site)]
...
pub fn shader(_attr: TokenStream, item: TokenStream) -> TokenStream {
  ...
  loop {
    let token = iter.next();
    if token.is_none() {
      return compile_error("invalid usage of 'rhythmc::shader', expected module");
    }
  ...
  if let TokenTree::Group(group) = iter.next().unwrap() {
    ...
    return compile_error("invalid usage of 'rhythmc::shader', failed to parse");
  }
